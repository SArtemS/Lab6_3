...
def test_get_class_distrib():
    assert get_class_distrib(data) == ((216, 24.24), (184, 20.65), (
        491, 55.11)), " Доли первого, второго и третьего класса"


def test_find_corr_sibsp_parch():
    assert find_corr_sibsp_parch(data) == (
        0.41
    ), " Коэффициент корреляции Пирсона между количеством супругов (SibSp) и количеством детей (Parch)"
...
def test_find_corr_class_survival():
    assert find_corr_class_survival(data) == (
        -0.34
    ), " Корреляция между классом, в котором пассажир ехал, и параметром Survived"
